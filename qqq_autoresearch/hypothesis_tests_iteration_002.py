"""Research-only tests for QQQ Risk Monitor iteration 002 hypotheses.
...
def _q(train: pd.DataFrame, col: str, p: float) -> float:
    if col not in train:
        return np.nan
    return _num(train, col).quantile(p)


def _ge(df: pd.DataFrame, col: str, val: float) -> pd.Series:
    if col not in df or pd.isna(val):
        return pd.Series(False, index=df.index)
    return _num(df, col) >= val
...
def add_forward_mdd_labels(df: pd.DataFrame, horizons=(15, 30, 60, 126)) -> pd.DataFrame:
    ...
    for horizon in horizons:
        ...
        for i in range(n):
            ...
            vals.append(float(np.min(fwd / arr[i] - 1.0)) if len(fwd) else np.nan)
        out[f"future_{horizon}bd_mdd"] = vals
    return out


def trigger_i002_h001(test: pd.DataFrame, train: pd.DataFrame) -> pd.Series:
    ...
    short_trend_pressure = _le(test, "price_vs_ma20", 0.0)
    return range_stress & vol_stress & downside_tape & short_trend_pressure


def trigger_i002_h002(test: pd.DataFrame, train: pd.DataFrame) -> pd.Series:
    credit_stress = (
    ...
    confirming_shock = (
        ...
        | _ge(test, "vix_percentile_252d", _q(train, "vix_percentile_252d", 0.80))
    ...
    return credit_stress & confirming_shock


def trigger_i002_h003(test: pd.DataFrame, train: pd.DataFrame) -> pd.Series:
    cap_weight_divergence = (
        _ge(test, "qqq_vs_qqqe_3m", _q(train, "qqq_vs_qqqe_3m", 0.75))
        | _ge(test, "qqq_vs_rsp_3m", _q(train, "qqq_vs_rsp_3m", 0.75))
    )
    semis_non_confirmation = (
        _ge(test, "qqq_vs_soxx_3m", _q(train, "qqq_vs_soxx_3m", 0.75))
        | _le(test, "smh_vs_qqq_3m", _q(train, "smh_vs_qqq_3m", 0.25))
        | _truth(test, "semis_failure_flag")
        | _truth(test, "qqq_up_soxx_down_20d")
    )
    breadth_or_mmdi_narrow = (
        _le(test, "mag7_breadth", _q(train, "mag7_breadth", 0.30))
        | _ge(test, "mmdi_leadership_narrowing_stress", _q(train, "mmdi_leadership_narrowing_stress", 0.75))
    )
    return cap_weight_divergence & semis_non_confirmation & breadth_or_mmdi_narrow


def trigger_i002_h004(test: pd.DataFrame, train: pd.DataFrame) -> pd.Series:
    ...
    renewed_tape_stress = (
        _ge(test, "range_20d_vs_252d", _q(train, "range_20d_vs_252d", 0.70))
        | _ge(test, "large_red_day_count_20d", _q(train, "large_red_day_count_20d", 0.75))
    )
    return apparent_repair & unresolved_damage & bounce_context & renewed_tape_stress


def trigger_i002_h005(test: pd.DataFrame, train: pd.DataFrame) -> pd.Series:
    ...
    component_stress = (
        _ge(test, "mmdi_vix_stress", _q(train, "mmdi_vix_stress", 0.70))
        | _ge(test, "mmdi_vxn_stress", _q(train, "mmdi_vxn_stress", 0.70))
        | _ge(test, "mmdi_realized_vol_stress", _q(train, "mmdi_realized_vol_stress", 0.75))
        | _ge(test, "mmdi_trend_damage", _q(train, "mmdi_trend_damage", 0.70))
        | _ge(test, "mmdi_leadership_narrowing_stress", _q(train, "mmdi_leadership_narrowing_stress", 0.75))
    )
    return r2_quiet & mmdi_elevated_or_accelerating & component_stress


TRIGGERS: dict[str, Callable[[pd.DataFrame, pd.DataFrame], pd.Series]] = {
    "I002_H001": trigger_i002_h001,
    "I002_H002": trigger_i002_h002,
    "I002_H003": trigger_i002_h003,
    "I002_H004": trigger_i002_h004,
    "I002_H005": trigger_i002_h005,
}
...
def load_daily_wide(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path)
...
def make_folds(df: pd.DataFrame, min_train_years: int = 8, min_test_days: int = 80) -> list[tuple[pd.Timestamp, pd.Timestamp]]:
    idx = pd.to_datetime(df.index)
    years = sorted(set(idx.year))
    folds: list[tuple[pd.Timestamp, pd.Timestamp]] = []
    for year in years:
        if year - years[0] < min_train_years:
            continue
        start = pd.Timestamp(f"{year}-01-01")
        end = pd.Timestamp(f"{year}-12-31")
        if ((idx >= start) & (idx <= end)).sum() >= min_test_days:
            folds.append((start, end))
    return folds


def _event_rows(frame: pd.DataFrame, hit: pd.Series, alert: pd.Series, horizon: int, hypothesis_id: str, year: int | None = None) -> list[dict]:
    hit = hit.fillna(False).astype(bool).reindex(frame.index, fill_value=False)
    alert = alert.fillna(False).astype(bool).reindex(frame.index, fill_value=False)
    starts_mask = hit & ~hit.shift(1, fill_value=False)
    rows: list[dict] = []
    for event_no, start in enumerate(frame.index[starts_mask], start=1):
        loc = frame.index.get_loc(start)
        left = max(0, loc - horizon)
        window = alert.iloc[left : loc + 1]
        alert_positions = np.flatnonzero(window.to_numpy(dtype=bool))
        covered = len(alert_positions) > 0
        lead_time = int(loc - (left + alert_positions[-1])) if covered else np.nan
        rows.append({
            "hypothesis_id": hypothesis_id,
            "year": year if year is not None else "",
            "event_no": event_no,
            "event_start": str(pd.Timestamp(start).date()),
            "covered": bool(covered),
            "lead_time_bd": lead_time,
        })
    return rows


def _summarize_events(frame: pd.DataFrame, hit: pd.Series, alert: pd.Series, horizon: int, hypothesis_id: str, year: int | None = None) -> tuple[dict, list[dict]]:
    rows = _event_rows(frame, hit, alert, horizon, hypothesis_id, year)
    total = len(rows)
    covered = sum(1 for row in rows if row["covered"])
    leads = [row["lead_time_bd"] for row in rows if row["covered"] and pd.notna(row["lead_time_bd"])]
    stats = {
        "events_covered": covered,
        "events_total": total,
        "event_coverage": covered / total if total else np.nan,
        "false_calm_events": total - covered,
        "median_lead_time_bd": float(np.median(leads)) if leads else np.nan,
        "mean_lead_time_bd": float(np.mean(leads)) if leads else np.nan,
    }
    return stats, rows


def _prior_alert_window(alert: pd.Series, horizon: int) -> pd.Series:
    return alert.fillna(False).astype(bool).rolling(horizon + 1, min_periods=1).max().astype(bool)
...
def run_tests(data_path: Path, hypotheses_path: Path, output_dir: Path, min_train_years: int = 8) -> dict:
    ...
    paths = {
        "summary_path": output_dir / "iteration_002_hypothesis_test_summary.csv",
        "folds_path": output_dir / "iteration_002_hypothesis_test_folds.csv",
        "signals_path": output_dir / "iteration_002_hypothesis_test_daily_signals.csv",
        "events_path": output_dir / "iteration_002_hypothesis_test_events.csv",
        "html_path": output_dir / "iteration_002_hypothesis_test_report.html",
        "json_path": output_dir / "iteration_002_hypothesis_test_summary.json",
    }
...
def write_html_report(summary: pd.DataFrame, folds: pd.DataFrame, events: pd.DataFrame, path: Path) -> None:
    ...
    """
    body = [
        "<h1>QQQ Hypothesis Test Report - Iteration 002</h1>",

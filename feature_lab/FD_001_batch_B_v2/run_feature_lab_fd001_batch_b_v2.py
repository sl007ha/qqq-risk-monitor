#!/usr/bin/env python
"""Run FD_001 Feature Lab Batch B v2 QA patch.

This research-only script replaces FREQ_003's single binary flag with
confirmed/possible/unknown flags while keeping FREQ_004 unchanged.
"""
...
FEATURE_ROLES = {
    ...
    "freq_003_false_calm_confirmed_flag": "candidate_hypothesis_design",
    "freq_003_false_calm_possible_flag": "candidate_hypothesis_design",
    "freq_003_false_calm_unknown_due_to_missing_flag": "descriptive_only",
...
def safe_bool_series(series: pd.Series) -> pd.Series:
    ...
    if pd.api.types.is_numeric_dtype(series):
        return series.map(lambda value: pd.NA if pd.isna(value) else bool(value)).astype("boolean")
...
def first_valid_date(frame: pd.DataFrame, column: str) -> str:
    ...
    return "" if valid.empty else str(valid.iloc[0].date())


def last_valid_date(frame: pd.DataFrame, column: str) -> str:
    valid = frame.loc[frame[column].notna(), "date"]
    return "" if valid.empty else str(valid.iloc[-1].date())
...
def build_freq_003(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    observed = true_count(components)
    missing = missing_count(components)
    quiet_true = quiet_state.eq(True).fillna(False)
    quiet_false = quiet_state.eq(False).fillna(False)

    observed_count = pd.Series(np.nan, index=df.index, dtype="float64")
    observed_count.loc[quiet_false] = 0.0
    observed_count.loc[quiet_true] = observed.loc[quiet_true].astype(float)

    missing_component_count = pd.Series(np.nan, index=df.index, dtype="float64")
    missing_component_count.loc[quiet_false] = 0.0
    missing_component_count.loc[quiet_true] = missing.loc[quiet_true].astype(float)

    possible_total = observed_count + missing_component_count
    confirmed = pd.Series(np.nan, index=df.index, dtype="float64")
    confirmed.loc[quiet_false] = 0.0
    confirmed.loc[quiet_true & (observed_count >= 3)] = 1.0
    confirmed.loc[quiet_true & (observed_count < 3) & (possible_total < 3)] = 0.0

    possible = pd.Series(np.nan, index=df.index, dtype="float64")
    possible.loc[quiet_false] = 0.0
    possible.loc[quiet_true & (possible_total >= 3)] = 1.0
    possible.loc[quiet_true & (possible_total < 3)] = 0.0

    unknown = pd.Series(0.0, index=df.index, dtype="float64")
    unknown.loc[quiet_state.isna()] = np.nan
    unknown.loc[quiet_true & (observed_count < 3) & (possible_total >= 3)] = 1.0

    outputs = pd.DataFrame(
        {
            "freq_003_quiet_state": boolean_to_float(quiet_state),
            "freq_003_false_calm_internal_deterioration_count": observed_count,
            "freq_003_false_calm_missing_component_count": missing_component_count,
            "freq_003_false_calm_confirmed_flag": confirmed,
            "freq_003_false_calm_possible_flag": possible,
            "freq_003_false_calm_unknown_due_to_missing_flag": unknown,
        },
        index=df.index,
    )
...
def build_freq_004(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    active_pressure_ready = active_true & pressure_count.notna()
    relapse_flag.loc[active_pressure_ready] = (pressure_count.loc[active_pressure_ready] >= 2).map(
        {True: 1.0, False: 0.0}
    )
    ...
    diagnostics = pd.concat(
        [pressure_components, pressure_missing_count.rename("repair_pressure_missing_component_count")],
        axis=1,
    )
    return outputs, diagnostics
        ...
        rows.append(
            {
                ...
                "column": column,
                "feature_role": FEATURE_ROLES[column],
...
def write_validation_summary(
    ...
    distributions = {
        "Quiet state": snapshot["freq_003_quiet_state"],
        "Observed deterioration count": snapshot["freq_003_false_calm_internal_deterioration_count"],
        "Missing component count": snapshot["freq_003_false_calm_missing_component_count"],
        "Confirmed flag": snapshot["freq_003_false_calm_confirmed_flag"],
        "Possible flag": snapshot["freq_003_false_calm_possible_flag"],
        "Unknown due to missing flag": snapshot["freq_003_false_calm_unknown_due_to_missing_flag"],
        "Repair episode active": snapshot["freq_004_repair_episode_active"],
        "Repair pressure count": snapshot["freq_004_repair_failure_pressure_count"],
        "Repair relapse flag": snapshot["freq_004_repair_relapse_flag"],
    }
    lines.extend(["", "## State And Count Distributions"])
    for title, series in distributions.items():
        lines.extend(["", f"{title}:"])
        for key, value in distribution(series).items():
    ...
    lines.extend(["", "FREQ_004 active repair days by year:"])
    for year, value in active_days_by_year(df["date"], snapshot["freq_004_repair_episode_active"]).items():
    ...
    manifest = {
        ...
        "features": {
            "FREQ_003": {
                ...
                "qa_patch_note": "Single binary flag replaced with confirmed, possible, and unknown-due-to-missing flags.",
            },
            "FREQ_004": {
                ...
                "formula_note": "Unchanged from Batch B.",
            },
        },
        "artifacts": [
